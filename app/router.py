...
@app.post("/azal/itinerary_generator/")
async def itinerary_generator(itinerary_generation_input: ItineraryGenerationInput):
    """
    Endpoint to generate an itinerary based on the provided input parameters.

    Args:
        itinerary_generation_input (ItineraryGenerationInput): The input parameters for itinerary generation.

    Returns:
        dict: The output generated by the agent based on the input parameters.

    Raises:
        HTTPException: If there is an error during the execution of the agent.
    """
    try:
        # Convert input model to dictionary
        in_params = itinerary_generation_input.model_dump()
        ...
        # Log the error and raise HTTPException with a message
        print(f"Error in itinerary_generator endpoint: {e}")
        raise HTTPException(status_code=500, detail="Internal Error, If the issue persists please call admin")
